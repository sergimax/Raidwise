...
export const ruMessages: Messages = {
  ...
  intro: {
    ...
    subtitle: "Три способа пользоваться приложением",
  ...
  characterEdit: {
    ...
    alsoOwnedHelper:
      "Предметы, доступные для экипировки, но не на персонаже в экспорте WowSims (в сумках или банке). Учитываются в подсказках для обоих спеков.",
  ...
  gearPickPanel: {
    ...
    itemsHelper:
      "Из активного BiS-списка: недостающие BiS и варианты в выбранных рейдах. Назначьте свои софты и чужие заявки — цвет подсказки показывает спрос (зелёный→красный), рядом расчёт, сколько софтов повышает шанс. Сброс — по строке или для всего блока.",
