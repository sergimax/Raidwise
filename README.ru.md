...
Копируемые строки набора персонажей без КД по отфильтрованным рейдам: фильтры мин. GS / роль / спеки, опционально **С апгрейдами**, и **Спеки** / **GS** в формате строки.
Фильтры хранятся в сессии; **Сбросить параметры** очищает их; клик по чипу выбранного рейда исключает ложные совпадения.
...
Опция **С апгрейдами** приглушает персонажей без BiS-целей; **Сбросить параметры** очищает фильтры панели.
...
Светлая/тёмная тема сохраняется локально; спокойный MUJI-хром с лёгкой точечной сеткой на странице и панелях; дизайн-токены в [docs/design/design-system.md](docs/design/design-system.md).
