...
Колонка **Тип** — размер (+ череп для героика); у строк — правка / удаление рейда.
...
Кнопка **Настройки** открывает управление данными: массовый сброс КД или удаление всех персонажей / подземелий / локальных BIS (с подтверждением).
...
Правила: максимум софтов (1–4) и система (**переролл** или **+100**).
...
**Копировать** выгружает список в виде строк `Слот: Предмет`.
...
Ячейки КД (скрываются, когда КД персонажа на рейд отмечен):
- <span style="color:#d97706">янтарный</span> — недостающий BiS (точный или обычный вариант)
- <span style="color:#0284c7">синий</span> — апгрейд по ilvl с учётом статов

При мейне и оффе ячейка делится пополам по оттенку.
Скрываемая легенда над таблицей объясняет цвета.
В тултипе лут по боссам и токены тир-сета для класса персонажа.
Предметы из «Также есть» (редактирование персонажа) учитываются в BiS/ilvl/токен-подсказках, даже если их нет в экспорте WowSims.
